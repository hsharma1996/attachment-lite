...
test.group("Attachment Options", (group) => {
  ...
  test("validateMimeType validates mime types correctly", async ({
    assert,
  }: TestContext) => {
    ...
    assert.doesNotThrow(() => {
      if (
        ...
        if (
          !textFile.validateMimeType({ allowedMimes: options.allowedMimes })
        ) {
          throw new Error(
    ...
    assert.throws(() => {
      if (
        ...
        if (
          !imageFile.validateMimeType({ allowedMimes: options.allowedMimes })
        ) {
          throw new Error(
            `Invalid MIME type for attachment. ` +
  ...
  test("computeUrl option is honored by the model", async ({
    assert,
  }: TestContext) => {
  ...
  test("attachment options are stored on the model", async ({
    assert,
  }: TestContext) => {
    ...
    assert.deepEqual(UserModel.$attachments.avatar.allowedMimes, [
      "image/jpeg",
      "image/png",
    ]);
  });

  test("setOptions method applies attachment options correctly", async ({
    assert,
  }: TestContext) => {
    // Create an attachment
    const file = new Attachment();
